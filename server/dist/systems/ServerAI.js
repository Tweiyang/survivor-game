    ...
    update(dt) {
        ...
        this.monsters.forEach((monster) => {
            ...
            if (dist > 1) {
                ...
                // 碰撞检测
                const resolved = this.physics.resolveMovement(monster.x, monster.y, targetX, targetY, radius);
                monster.x = resolved.x;
                monster.y = resolved.y;
            }
            // 接触伤害检测
            for (const p of alivePlayers) {
                if (this.physics.circleVsCircle(monster.x, monster.y, radius, p.x, p.y, p.radius)) {
