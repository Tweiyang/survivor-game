...
function createGameEnv(playerCount = 1, characterId = 'ranger') {
    const { characters, levels, formulas } = loadConfigs();
    const state = new BattleState_1.BattleRoomState();
    // 创建玩家
    const tileSize = levels.levels[0]?.tileSize || 64;
    const startRoom = levels.levels[0]?.rooms?.find((r) => r.type === 'start');
    ...
    // 初始化子系统
    const difficultyScaler = new DifficultyScaler_1.DifficultyScaler({
    ...
    const physics = new ServerPhysics_1.ServerPhysics();
    ...
    const spawner = new ServerSpawner_1.ServerSpawner(state.monsters, physics);
    spawner.setDifficultyMultiplier(difficultyScaler.multiplier);
    spawner.loadLevel(levelConfig);
    const ai = new ServerAI_1.ServerAI(state.monsters, state.players, physics);
    const combat = new ServerCombat_1.ServerCombat(state.players, state.monsters, state.projectiles, state.drops, physics);
    combat.loadConfigs(formulas, characters);
    const broadcasts = [];
    const monsterKills = [];
    // 挂接回调（回调引用闭包中的 monsterKills 数组，清空时用 .length=0 保持引用）
    combat.onMonsterKill = (monsterId, monsterType) => {
        monsterKills.push({ monsterId, monsterType });
    };
    combat.onDamageEvent = () => { };
    combat.onSfxEvent = () => { };
    combat.onLevelUp = () => { };
    ai.onContactDamage = (playerId, damage, monsterId) => {
 ...
 */
function simulateTicks(env, seconds, tickRate = 0.05) {
 ...
 * 将玩家传送到指定世界坐标
 */
function teleportPlayer(env, playerId, x, y) {
 ...
 * 将玩家传送到最近的怪物旁边（攻击范围内）
 */
function teleportToNearestMonster(env, playerId) {
    const player = env.state.players.get(playerId);
    if (!player)
        return;
    let nearest = null;
 ...
 * 直接击杀所有存活怪物（模拟快速通关）
 */
function killAllMonsters(env, killerPlayerId) {
    const toKill = [];
 ...
 * 直接击杀指定类型的怪物
 */
function killMonstersByType(env, type, killerPlayerId, count) {
    let killed = 0;
    env.state.monsters.forEach((m) => {
        if (!m.alive)
            return;
        if (m.monsterType !== type)
            return;
        if (count !== undefined && killed >= count)
            return;
        m.alive = false;
        m.hp = 0;
        const player = env.state.players.get(killerPlayerId);
 ...
 */
function checkLevelProgress(env) {
 ...
 */
function loadNextLevel(env, levelIdx) {
    const ls = env.state.levelState;
...
function test1_MonsterSpawning() {
    ...
    env.state.monsters.forEach(m => {
        if (m.alive) {
            aliveCount++;
            if (m.monsterType.startsWith('boss'))
                bossCount++;
        }
    });
...
function test4_BossKillAndLevelTransition() {
    ...
    if (totalLevels > 1) {
        ...
        env.state.monsters.forEach(m => { if (m.alive)
            level2Monsters++; });
...
function test6_PlayerDeath() {
    ...
    // 检查全灭判定
    let allDead = true;
    env.state.players.forEach(p => { if (p.alive)
        allDead = false; });
...
function test8_DifficultyScaling() {
    section('TEST 8: 难度缩放');
    const scaler1 = new DifficultyScaler_1.DifficultyScaler({
        scalingFormula: 'linear', baseMultiplier: 1.0, perPlayerAdd: 0.5,
        fields: { monsterHp: true, monsterCount: true, monsterDamage: false, bossHp: true }
    });
...
main();
//# sourceMappingURL=GameFlowTest.js.map